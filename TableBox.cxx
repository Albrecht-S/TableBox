...
TableBox::TableBox(int _x, int _y, int _w, int _h)
...
void TableBox::layout() {
  ...
  for (uint j = 0; j < t->rows; j++) {
    for (uint i = 0; i < t->cols; i++) {
      // w
      if (x_fixed_size_item[i] > 0)
        Wv = x_fixed_size_item[i];
      else
        Wv = x_expand_size_item;

      // x span
      if (t->cell[j][i].x_span > 1) {
        for (uint k = 1; k < t->cell[j][i].x_span; k++) {
          if (x_fixed_size_item[i + k] > 0)
            Wv += x_fixed_size_item[i + k] + t->gap->w;
          else
            Wv += x_expand_size_item + t->gap->w;
        }
      }

      // x fill
      if (t->cell[j][i].x_fill)
        Wm = Wv;
      else if (Wv > t->cell[j][i].w)
        Wm = t->cell[j][i].w;
      else
        Wm = Wv;
      ...
      // h
      if (y_fixed_size_item[j] > 0)
        Hv = y_fixed_size_item[j];
      else
        Hv = y_expand_size_item;

      // y span
      if (t->cell[j][i].y_span > 1) {
        for (uint k = 1; k < t->cell[j][i].y_span; k++) {
          if (y_fixed_size_item[j + k] > 0)
            Hv += y_fixed_size_item[j + k] + t->gap->h;
          else
            Hv += y_expand_size_item + t->gap->h;
        }
      }

      // y fill
      if (t->cell[j][i].y_fill)
        Hm = Hv;
      else if (Hv > t->cell[j][i].h)
        Hm = t->cell[j][i].h;
      else
        Hm = Hv;
      ...
      // Albrecht: The following code is deactivated in FLTK 1.4. The only
      // thing we need to do is resize() and this has already been done above.
      // Replacing layout() with init_sizes() would be wrong.
#if (0)
      if (t->cell[j][i].o && t->cell[j][i].o->as_group() && !t->cell[j][i].o->as_window()) {
        Fl_Group *grp = (Fl_Group *)(t->cell[j][i].o);
        grp->init_sizes(); // layout() in fltk2
      }
#endif

      // update X
      if (x_fixed_size_item[i] > 0)
        _x += x_fixed_size_item[i] + t->gap->w;
      else
        _x += x_expand_size_item + t->gap->w;
    }
    ...
    // update Y
    if (y_fixed_size_item[j] > 0)
      _y += y_fixed_size_item[j] + t->gap->h;
    else
      _y += y_expand_size_item + t->gap->h;
  }
}
...
Fl_Widget *TableBox::Widget(uint _row, uint _col) {
...
void TableBox::Widget(Fl_Widget *_widget, uint _row, uint _col) {
...
void TableBox::Attach(Fl_Widget *_widget,
                      uint _col, uint _row,
                      uint _w, uint _h,
...
void TableBox::Add(Fl_Widget *_widget,
                   uint _w, uint _h,
                   uint _x_prop, uint _y_prop,
                   uint _x_span, uint _y_span,
                   float _x_align, float _y_align) {
